...
uint get_total_signal_no() {return 1849;}

uint get_number_of_components() {return 7;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 1826;}
...
{
char* temp = Fr_element2str(&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[2])) + 7)]);
printf("%s",temp);
delete [] temp;
}
{
...
{
char* temp = Fr_element2str(&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[2])) + 0)]].signalStart + 0]);
printf("%s",temp);
delete [] temp;
}
{
...
Fr_add(&expaux[1],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[2])) + 7)],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[2])) + 1)]); // line circom 157
Fr_add(&expaux[0],&expaux[1],&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[2])) + 0)]].signalStart + 0]); // line circom 157
{
...
{
printf("r[i]+mims[i].out: ");
}
{
printf(" ");
}
Fr_add(&expaux[0],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[2])) + 7)],&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[2])) + 0)]].signalStart + 0]); // line circom 158
{
char* temp = Fr_element2str(&expaux[0]);
printf("%s",temp);
delete [] temp;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "MultiMiMC7";
...
{
char* temp = Fr_element2str(&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 0]);
printf("%s",temp);
delete [] temp;
}
{
