...
// check help/createAccountZero.js for more details about Zero Account
...
exports.createUserL1 = (prvkey_hex) => {
    const wallet = new ethers.Wallet(prvkey_hex);
    return wallet;
}


exports.getL1Address = (prvkey) => {
    return (new ethers.Wallet(prvkey)).address;
}


exports.createUserL2 = (index, prvkey_hex, mimc, eddsa) => {
    const _prvkey_uint8array = mimc.F.e(prvkey_hex, 16);
    const pubkey_Ed = eddsa.prv2pub(_prvkey_uint8array);
    const _l1Address = (new ethers.Wallet(prvkey_hex)).address;
    return {
        originalPrvkey: prvkey_hex,
        l1Address: _l1Address,
        index: index,
        prvkey: uint8Array2Hex(_prvkey_uint8array),
        pubkeyX: uint8Array2Hex(pubkey_Ed[0]),
        pubkeyY: uint8Array2Hex(pubkey_Ed[1])
    }
}
