...
contract Middleware {
    ...
    event _e_process_deposit_register(bytes32 deposit_register_root);
    event _e_process_deposit_existence(bytes32 deposit_existence_root);
    event _e_transfer(address _from, address _to, uint _amount);
    event _e_withdraw();
    event _e_valid_proof();
    event _e_invalid_proof();
    event _e_tracking(string text, bytes32 _value);
    event _e_tracking_num(string text, uint num);
    event _e_tracking_addr(string text, address addr);
