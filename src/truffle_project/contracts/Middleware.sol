...
contract Middleware {
    ...
    function _deposit_register(address l1_addr, address l2_addr, uint amount) private returns(uint) {
        ...
        emit _e_deposit_register_root_change(keccak256(abi.encodePacked(l1_addr, l2_addr, amount)));
    ...
    function _deposit_existence(address l1_addr, address l2_addr, uint amount) private returns(uint) {
        ...
        emit _e_deposit_existence_root_change(keccak256(abi.encodePacked(l1_addr, l2_addr, amount)));
